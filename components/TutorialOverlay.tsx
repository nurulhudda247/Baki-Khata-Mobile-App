      ...
      <Animated.View style={[styles.infoBox, { top: infoTop, opacity: contentOpacity }]} pointerEvents="box-none">
        ...
        <View style={[styles.card, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <Text style={[styles.title, { color: theme.colors.textPrimary, fontSize: sfs(22), marginBottom: 0, flex: 1 }]}>{step.title}</Text>
            <Text style={{ color: theme.colors.primary, fontSize: sfs(14), fontWeight: 'bold' }}>{currentStep + 1} / {steps.length}</Text>
          </View>
