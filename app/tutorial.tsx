...
export default function TutorialScreen() {
  ...
  const swipeHintRef = useRef<View>(null);
  ...
  const steps = [
    ...
    {
      id: 'home_swipe',
      targetRect: targets['swipeHint'] || null,
      title: t('tutorial.steps.swipeActions.title'),
      description: t('tutorial.steps.swipeActions.desc'),
    ...
    {
      id: 'payment_presets',
      targetRect: targets['presets'] || null,
      title: t('tutorial.steps.paymentPresets.title'),
      description: t('tutorial.steps.paymentPresets.desc'),
      arrowDirection: 'up' as const,
    },
    {
      id: 'product_tap',
    ...
    {
      id: 'purchase_qty',
      targetRect: targets['qtyControl'] || null,
      title: t('tutorial.steps.quantityControls.title'),
      description: t('tutorial.steps.quantityControls.desc'),
      arrowDirection: 'up' as const,
    },
    {
      id: 'purchase_save',
  ...
  const handleNext = () => {
    if (currentStep < steps.length - 1) {
      ...
      setTimeout(() => {
        ...
        if (nextId === 'payment_presets') onMeasure('presets', presetsRef);
        if (nextId === 'product_tap') onMeasure('productCard', productCardRef);
        if (nextId === 'purchase_qty') onMeasure('qtyControl', qtyControlRef);
  ...
  const renderMockHome = () => {
    const isSwipeStep = steps[currentStep]?.id === 'home_swipe';
    return (
          ...
          <View style={{ position: 'relative' }}>
            {isSwipeStep && (
              <View 
                ref={swipeHintRef}
                collapsable={false}
                onLayout={() => onMeasure('swipeHint', swipeHintRef)}
            ...
            <View 
              ...
              style={[
                ...
                isSwipeStep && { transform: [{ translateX: 90 }] }
              ]}
            >
  ...
  const renderMockShopDetails = () => {
    const isPaymentStep = steps[currentStep]?.id === 'payment_presets';
    return (
      <View style={styles.mockScreen}>
        ...
        <View style={styles.contentPadding}>
          ...
          {isPaymentStep && (
            <View style={{ position: 'absolute', top: -50, left: 0, right: 0, bottom: -200, backgroundColor: 'rgba(0,0,0,0.7)', zIndex: 100, justifyContent: 'flex-end', padding: 24, paddingBottom: 40, borderTopLeftRadius: 32, borderTopRightRadius: 32 }}>
              <View style={[styles.modalContentMock, { backgroundColor: theme.colors.surface }]}>
                ...
                <View style={styles.modalHeaderMock}>
                  <Text style={[styles.modalTitleMock, { color: theme.colors.textPrimary }]}>{t('shop.recordPayment')}</Text>
                ...
                <View style={styles.modalBodyMock}>
                  <Text style={[styles.inputLabelMock, { color: theme.colors.textSecondary }]}>{t('shop.paymentAmount')}</Text>
                  <View style={[styles.modalInputMock, { backgroundColor: theme.colors.background, borderColor: theme.colors.primary }]}><Text style={{ color: theme.colors.textPrimary, fontSize: sfs(16) }}>100</Text></View>
                  <View 
                    ref={presetsRef}
                    onLayout={() => onMeasure('presets', presetsRef)}
                    style={styles.presetContainerMock}
                  >
                    {[50, 100, 200, 500].map(amt => (
                      <View key={amt} style={[styles.presetChipMock, { backgroundColor: amt === 100 ? theme.colors.primary + '15' : theme.colors.background, borderColor: amt === 100 ? theme.colors.primary : theme.colors.border }]}>
                        <Text style={{ color: amt === 100 ? theme.colors.primary : theme.colors.textPrimary, fontWeight: 'bold' }}>৳{amt}</Text>
                      </View>
                    ))}
                  </View>
                </View>
  ...
  const renderMockPurchaseEntry = () => (
    <View style={{ flex: 1 }}>
      {renderMockShopDetails()}
      <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.7)', zIndex: 100, justifyContent: 'flex-end', padding: 24, paddingBottom: 40, borderTopLeftRadius: 32, borderTopRightRadius: 32 }}>
        <View style={[styles.modalContentMock, { backgroundColor: theme.colors.surface }]}>
          <View style={styles.modalIndicatorMock} />
          <View style={styles.modalHeaderMock}>
          ...
          <View style={styles.modalBodyMock}>
            ...
            <View 
              ...
              style={styles.mockQtyContainer}
            >
              <View style={[styles.qtyBtn, { backgroundColor: theme.colors.background, borderColor: theme.colors.border }]}><Ionicons name="remove" size={sfs(24)} color={theme.colors.textPrimary} /></View>
              <View style={[styles.modalInputMock, { flex: 1, backgroundColor: theme.colors.background, borderColor: theme.colors.primary, marginBottom: 0, alignItems: 'center' }]}><Text style={{ color: theme.colors.textPrimary, fontSize: sfs(20), fontWeight: 'bold' }}>2</Text></View>
              <View style={[styles.qtyBtn, { backgroundColor: theme.colors.background, borderColor: theme.colors.border }]}><Ionicons name="add" size={sfs(24)} color={theme.colors.textPrimary} /></View>
            </View>
          </View>
          <View 
            ref={savePurchaseRef}
    ...
    <View style={styles.mockScreen}>
      ...
      <View style={styles.contentPadding}>
        <View 
          ...
          style={[styles.productCardListItemMock, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}
        >
          <View style={[styles.productIcon, { backgroundColor: theme.colors.primary + '15' }]}><Ionicons name="fast-food" size={sfs(24)} color={theme.colors.primary} /></View>
          <View style={{ flex: 1, marginLeft: 16 }}>
  ...
  const renderCurrentMock = () => {
    ...
    if (stepId === 'home_add' || stepId === 'home_card' || stepId === 'tutorial_restart' || stepId === 'home_swipe') return renderMockHome();
    if (stepId === 'shop_record' || stepId === 'shop_manage' || stepId === 'product_tap' || stepId === 'payment_presets') return renderMockShopDetails();
    if (stepId === 'purchase_save' || stepId === 'purchase_qty') return renderMockPurchaseEntry();
...
const getStyles = (theme: any, sfs: any) => StyleSheet.create({
  ...
  shopName: { fontSize: sfs(18), fontWeight: 'bold' },
  ...
  headerShopName: { fontSize: sfs(18), fontWeight: 'bold', marginLeft: 16 },
  ...
  finishTitle: { fontSize: sfs(24), fontWeight: 'bold', marginBottom: 12, textAlign: 'center' },
  ...
  leftActionsContainerMock: {
    position: 'absolute', left: 0, top: 0, bottom: 0, width: 80, height: '83%', marginRight: 10, gap: 8, zIndex: 0,
  },
  swipeActionMockBtn: {
    flex: 1,
  ...
  shopCardHeaderMock: { flexDirection: 'row', alignItems: 'center', marginBottom: 16 },
  shopIconMock: { width: 54, height: 54, borderRadius: 16, justifyContent: 'center', alignItems: 'center', marginRight: 16 },
  shopCardFooterMock: { flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center', paddingTop: 16, borderTopWidth: 1 },
  modalContentMock: { borderTopLeftRadius: 32, borderTopRightRadius: 32, padding: 24, paddingBottom: 40, width: '100%' },
  ...
  mockQtyContainer: { flexDirection: 'row', alignItems: 'center', gap: 16, marginBottom: 16 },
  qtyBtn: { width: 60, height: 60, borderRadius: 16, justifyContent: 'center', alignItems: 'center', borderWidth: 1 },
  presetContainerMock: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  presetChipMock: { paddingHorizontal: 16, paddingVertical: 10, borderRadius: 12, borderWidth: 1 },
  productCardListItemMock: { flexDirection: 'row', alignItems: 'center', padding: 16, borderRadius: 20, borderWidth: 1, marginBottom: 12 },
});
