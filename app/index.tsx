...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, RefreshControl, TextInput, Platform, Alert, Linking } from 'react-native';
...
const getStyles = (theme: any, sfs: any, insets: any) => StyleSheet.create({
  ...
  leftActionsContainer: { width: 80, height: '100%', marginRight: 10, gap: 8 },
  swipeActionBtn: { flex: 1, justifyContent: 'center', alignItems: 'center', borderRadius: 16 },
  swipeActionText: { color: 'white', fontSize: sfs(10), fontWeight: 'bold', marginTop: 2 },
...
export default function Home() {
  ...
  const { userProfile, isLoading: appLoading } = useAppContext();
  ...
  useFocusEffect(useCallback(() => { fetchShops(); }, [userProfile]));
  ...
  const handleConfirmDelete = async () => {
    if (shopToDelete) {
      try {
        ...
        Alert.alert(t('common.error'), t('home.deleteShopError'));
        setConfirmVisible(false);
      }
    }
  };
  ...
  ), [closeSwipeable, theme.colors.danger, sfs, t, styles.deleteAction, styles.deleteActionText]);

  const renderLeftActions = useCallback((item: Shop) => {
  ...
  }, [closeSwipeable, theme.colors.success, sfs, t, styles.leftActionsContainer, styles.swipeActionBtn, styles.swipeActionText]);
  ...
  // Don't redirect to login if Firebase user exists or app is still loading profile
  if (!isLoading && !userProfile && !isGuest && !user) return <Redirect href="/(auth)/login" />;
  
  // Only redirect to tutorial if profile is loaded and has_seen_tutorial is explicitly 0
  if (!isLoading && userProfile && userProfile.has_seen_tutorial === 0 && !isGuest) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={styles.header}>
        <View>
          <Text style={[styles.greeting, { color: theme.colors.textSecondary, fontSize: sfs(16) }]}>{t('home.hello') || 'Hello'},</Text>
          <Text style={[styles.userName, { color: theme.colors.textPrimary, fontSize: sfs(24) }]}>
            {userProfile?.name ? String(userProfile.name) : (user?.displayName || 'User')}
          </Text>
        </View>
        <TouchableOpacity
