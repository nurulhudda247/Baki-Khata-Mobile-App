...
const RootLayoutContent = () => {
  ...
  useEffect(() => {
    // Wait for auth to finish loading and fonts to be ready
    if (isAuthLoading || !fontsLoaded) return;

    const inAuthGroup = segments[0] === '(auth)';

    // Use a small delay or requestAnimationFrame to ensure the navigator is mounted
    const timeout = setTimeout(() => {
      try {
        if (!user && !isGuest && !inAuthGroup) {
          ...
          // Logged in users cannot see auth pages
          router.replace('/');
        }
      } catch (e) {
        console.warn('Navigation not ready yet, will retry...', e);
      }
    }, 1);

    return () => clearTimeout(timeout);
  }, [user, isGuest, isAuthLoading, segments, fontsLoaded]);
