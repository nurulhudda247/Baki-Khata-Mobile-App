...
export default function CustomerDetail() {
  ...
  const fetchData = async (refresh = true) => {
    ...
    try {
      ...
      const [customerData, transactions, payments, balanceData] = await Promise.all([
        getCustomerById(cId),
        getTransactionsByCustomer(cId, LIMIT, 0),
        getPaymentsByCustomer(cId, LIMIT, 0),
      ...
      const combined = [
        ...transactions.map((t: any) => ({ ...t, type: 'baki' as const, amount: t.total_amount, date: t.transaction_date })),
        ...payments.map((p: any) => ({ ...p, type: 'payment' as const, amount: p.amount, date: p.payment_date }))
      ...
      if (transactions.length < LIMIT && payments.length < LIMIT) setHasMore(false);
    } catch (e) {
  ...
  const loadMoreData = async () => {
    ...
    const [moreTransactions, morePayments] = await Promise.all([
      getTransactionsByCustomer(cId, LIMIT, nextPage * LIMIT),
      getPaymentsByCustomer(cId, LIMIT, nextPage * LIMIT)
    ]);
    
    if (moreTransactions.length > 0 || morePayments.length > 0) {
      const moreCombined = [
        ...moreTransactions.map((t: any) => ({ ...t, type: 'baki' as const, amount: t.total_amount, date: t.transaction_date })),
        ...morePayments.map((p: any) => ({ ...p, type: 'payment' as const, amount: p.amount, date: p.payment_date }))
      ];
      
      setHistory(prev => {
        const newHistory = [...prev, ...moreCombined];
        return newHistory.sort((a, b) => new Date((b as any).date).getTime() - new Date((a as any).date).getTime() || new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
      });
      setPage(nextPage);
    }
    
    if (moreTransactions.length < LIMIT && morePayments.length < LIMIT) setHasMore(false);
  };
